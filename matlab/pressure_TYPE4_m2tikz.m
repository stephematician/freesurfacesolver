%% Print out the type 4 solutions to pressure disturbances
%
...
MIN_HORZ_VEL = 0.03; % works ok enough for F=1.262, 0.02 for others
...
try
  ...
  a = dir('pressure_A*_U003_TYPE4.m');
  b = dir('pressure_A*_U003_TYPE4.mat');
  a1 = cell(1,length(a));
  for k=1:length(a)
...
for A = Avec
  ...
  try
    ...
    ['if ~exist(''data_A%s'', ''var'') || RECOMPUTE\n' ...
     '  if exist(''pressure_A%s_U003_TYPE4.m'', ''file'') || ...\n' ...
     '     exist(''pressure_A%s_U003_TYPE4.mat'', ''file'')\n' ...
     '    data_A%s = convert_and_load(''pressure_A%s_U003_TYPE4'');\n' ...
     '  end\n' ...
     'end\n' ...
