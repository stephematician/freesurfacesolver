...
MIN_HORZ_VEL = 0.02; % works good for all of these (N=1400 for 1.262)
% note: must be aware of make change to batch_plot_m2tikz for F=1.262 plot

disp('---------------------- Process Gaussian Pressure : TYPE 2 Solution');
disp(' ');
...
try
  ...
  a = dir('pressure_F*_U002_TYPE2.m');
  b = dir('pressure_F*_U002_TYPE2.mat');
  a1 = cell(1,length(a));
  for k=1:length(a)
...
for F = Fvec
  ...
  try
    ...
    ['if ~exist(''data_F%s'', ''var'') || RECOMPUTE\n' ...
     '  if exist(''pressure_F%s_U002_TYPE2.m'', ''file'') || ...\n' ...
     '     exist(''pressure_F%s_U002_TYPE2.mat'', ''file'')\n' ...
     '    data_F%s = convert_and_load(''pressure_F%s_U002_TYPE2'');\n' ...
     '  end\n' ...
     'end\n' ...
