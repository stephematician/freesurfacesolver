%% Print out the type 2 solutions to hyperbolic function topography
%
% Stephen Wade 02/12/2014
%
% Adapted from htopography_TYPE3_m2tikz
%
...
disp('------------------ Process hyperbolic topography : TYPE 2 Solution');
disp(' ');
...
try
  ...
  a = dir(['htopography_A*_U', ustr, '_TYPE2.m']);
  b = dir(['htopography_A*_U', ustr, '_TYPE2.mat']);
  a1 = cell(1,length(a));
  for k=1:length(a)
...
for A = Avec
  ...
  try
    ...
    ['if ~exist(''data_A%s'', ''var'') || RECOMPUTE\n' ...
     '  if exist(''htopography_A%s_U%s_TYPE2.m'', ''file'') || ...\n' ...
     '     exist(''htopography_A%s_U%s_TYPE2.mat'', ''file'')\n' ...
     '    data_A%s = convert_and_load(''htopography_A%s_U%s_TYPE2'');\n' ...
     '  end\n' ...
     'end\n' ...
