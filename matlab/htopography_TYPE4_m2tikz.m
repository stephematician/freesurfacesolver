...
disp('------------------ Process hyperbolic topography : TYPE 4 Solution');
disp(' ');
...
try
  ...
  a = dir(['htopography_A*_U', ustr, '_TYPE4.m']);
  b = dir(['htopography_A*_U', ustr, '_TYPE4.mat']);
  a1 = cell(1,length(a));
  for k=1:length(a)
...
for A = Avec
  ...
  try
    ...
    ['if ~exist(''data_A%s'', ''var'') || RECOMPUTE\n' ...
     '  if exist(''htopography_A%s_U%s_TYPE4.m'', ''file'') || ...\n' ...
     '     exist(''htopography_A%s_U%s_TYPE4.mat'', ''file'')\n' ...
     '    data_A%s = convert_and_load(''htopography_A%s_U%s_TYPE4'');\n' ...
     '  end\n' ...
     'end\n' ...
