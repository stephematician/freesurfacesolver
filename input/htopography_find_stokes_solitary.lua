stokes_single_tables = function(inN, inN_SUB_length)
...
find_stokes_solitary = function(inFROUDE, inB, inL, inU_MIN, inPHI_LIM, inN, inCLUSTER_VAL)
  ...
  local helperA         = -0.01
  local helperETA0      = (helperFROUDE - 1)
  local display_initial = false

  io.write('\n-------------------------------------\n')
  io.write(' Running script FIND_STOKES_SOLITARY \n')
  io.write('-------------------------------------\n')
  ...
  io.write('to a wave with A = ', helperA, '. Then find a  sol. wave solution with\n')
  io.write('U_MIN = ', inU_MIN, '. Then traverse to a sol. wave solution with\n')
  io.write('F = ', inFROUDE, '.\n')
  io.flush()
  ...
  if res ~= continuator.CONVERGED then
     ...
     error('Could not find initial tanh topography solution.')
  end
    ...
    if initial_step then
      local nsurf = surface.unpack_vec(freesurface.unknowns,
                                    ...
                                    freesurface.is_continued,
                                       freesurface.ind_table)
      local lsgn = 1
      if inU_MIN < csurf.U_MIN then
        lsgn = -1
      end
      if lsgn*(nsurf.U_MIN) < lsgn*(csurf.U_MIN) then
        freesurface:reset_interp()
        io.write('    Started in wrong direction, attempting to switch.\n')
        if direction == continuator.FORWARD then
          direction = continuator.BACKWARD
  ...
  local initial_stokes = surface.unpack_vec(freesurface.unknowns,
                                             freesurface.initial,
                                             freesurface.is_free,
                                        freesurface.is_continued,
                                           freesurface.ind_table)
 
  initial_stokes.U_MIN = inU_MIN

  io.write('  Parameters for helper stokes solitary solution,\n')
  io.write('         N : ')
  for k,v in pairs(initial_stokes.N_SUB) do
     io.write(' ' .. tostring(v))
  end
  io.write(',\n')
  io.write('         A : ', initial_stokes.A, ',\n')
  io.write('         B : ', initial_stokes.B, ',\n')
  io.write('         D : ', initial_stokes.D, ',\n')
  io.write('    LAMBDA : ', initial_stokes.LAMBDA, ',\n')
  io.write('      ETA0 : ', initial_stokes.ETA0, ',\n')
  io.write('     PHI_C : ', initial_stokes.PHI_C, '.\n\n')

  local is_free_stokes = htopography_grid.deepcopy(freesurface.is_free)
  local is_continued_stokes = htopography_grid.deepcopy(freesurface.is_continued)

  is_free_stokes.A = true
  is_free_stokes.U_MIN = false
  is_continued_stokes.FROUDE = true
  is_continued_stokes.U_MIN = false

  io.write('  Creating new surface object...')
  io.flush()

  freesurface = surface.new(   initial_stokes,
                               is_free_stokes,
                          is_continued_stokes,
            htopography_residuals.residual_hs,
      htopography_residuals.compute_output_hs)
  ...
  if res ~= continuator.CONVERGED then
     ...
     error('Could not find near stokes solitary wave solution.')
  end

  io.write('Begin continuation with respect to F.\n')

  freesurface:set_length_vars({'FROUDE', 'A'})
  ...
  for i = 1, num_continuate do
    ...
    if initial_step then
      ...
      if lsgn*(nsurf.FROUDE) < lsgn*(csurf.FROUDE) then
        freesurface:reset_interp()
        io.write('    Started in wrong direction, attempting to switch.\n')
        if direction == continuator.FORWARD then
          direction = continuator.BACKWARD
  ...
  initial_sol.FROUDE = inFROUDE

  local is_free_sol = htopography_grid.deepcopy(freesurface.is_free)
  local is_continued_sol = htopography_grid.deepcopy(freesurface.is_continued)

  is_continued_sol.A = true
  is_continued_sol.FROUDE = false

  return initial_sol, is_free_sol, is_continued_sol
