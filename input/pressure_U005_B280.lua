...
out_file_sl1 = 'output/pressure_U005_SL1_B280.m'
out_file_sl2 = 'output/pressure_U005_SL2_B280.m'
out_file_dl1 = 'output/pressure_U005_DL1_B280.m'
out_file_dl2 = 'output/pressure_U005_DL2_B280.m'
...
of = io.open(out_file_dl1, "r")

if not of then
   freesurface:create_file(out_file_dl1)
else
  if OVERWRITE then
    of:close()
    os.remove(out_file_dl1)
    freesurface:create_file(out_file_dl1) 
  end
end
...
freesurface:append_file(out_file_dl1)
...
for i = 1, num_continuate, 1 do
  ...
  if (not initial_step) then
    ...
    if nsurf.ETA0 < csurf.ETA0 then
      freesurface:reset_interp()
      io.write('  Seem to be going wrong direction, attempting to switch.\n')
      if direction == continuator.FORWARD then
        direction = continuator.BACKWARD
      else
        direction = continuator.FORWARD
...
of = io.open(out_file_dl2, "r")

if not of then
   freesurface:create_file(out_file_dl2)
else
  if OVERWRITE then
    of:close()
    os.remove(out_file_dl2)
    freesurface:create_file(out_file_dl2) 
  end
end
...
for i = 1, num_continuate, 1 do
  ...
  freesurface:append_file(out_file_dl2)
  ...
  if (not initial_step) then
    ...
    if (csurf.A > LIMIT_DOUBLE_A) then
       mesgt.body = mesgt.body .. 
                    'Finished doube-hump solutions with a small value of A.\n' ..
	            'Took ' .. (i+1) .. ' iterations.\n'
      break
    end
