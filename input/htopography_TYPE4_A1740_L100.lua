...
19/01/2015  Added re-starting functions and updated scripts.
]]--
...
dofile('input/htopography_find_type4_double.lua')

EMAILME = false
OVERWRITE = false

N_HI_RES = 1200 -- 1400
N_LO_RES = 300 -- 600 should do it.
...
ASTR = string.format('%03u',round(math.abs(TYPE4_A*1000)))
USTR = string.format('%03u',round(STOKES_U_MIN*100))

in_script  = 'input/htopography_type4_double_init.lua' -- alt?
...
if not of then
  ...
  if OVERWRITE then
    ...
    io.flush()
              hi_res_dbl,
         free_hi_res_dbl, 
    continued_hi_res_dbl = htopography_load.load_last(out_file,
                                           type4_double_tables,
                                                      N_HI_RES)
  end
end
...
num_continuate = 400
ds = 0.00435030419854153 -- 0.00510296051167363 -- 0.00514842199 larger omega -- 0.02
