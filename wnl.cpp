...
extern "C" int luaopen_wnl(lua_State *);
...
extern "C" int luaopen_wnl(lua_State *L) {
...
double delta_approx_dx(double x, double B)
{
  return -B * B * B * (2.0 * x) * exp(-B * B * x * x) / sqrt(pi);
}
...
int wnl_J(double x, const double *y, double *dfdy, double *dfdx, void *params) 
{
  ...
  dfdx[0] = 0.0;
  dfdx[1] = -(3.0 * A * delta_approx_dx(x, B));
